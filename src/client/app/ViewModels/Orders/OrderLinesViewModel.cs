				...
				.Select(_ =>
				{
					...
					lines.Each(l => {
						...
						if (l.Order.IsAddressExists())
							l.CalculateRetailCost(Settings.Value, Shell?.SpecialMarkupProducts.Value, User, l.Order.Address);
					});
		...
		protected void CalculateOrderLine()
		{
			foreach (var line in Lines.Value) {
				if (line.Order.IsAddressExists())
					line.CalculateRetailCost(Settings.Value, Shell?.SpecialMarkupProducts.Value, User, line.Order.Address);
			}
		}
