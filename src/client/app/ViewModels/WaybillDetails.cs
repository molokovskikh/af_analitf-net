	...
	public class WaybillDetails : BaseScreen
	{
		...
		private List<TagPrintable> PrintableLinesForTag()
		{
			return PrintableLines().Select(x => new TagPrintable() {
				...
				ClientName = Settings.Value.Waybills.FirstOrDefault(y => y.BelongsToAddress.Id == Address.Id)?.Name,
				AddressName = Settings.Value.Waybills.FirstOrDefault(y => y.BelongsToAddress.Id == Address.Id)?.Address,
				Producer = x.Producer,
